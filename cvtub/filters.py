...
from cvtub.utils import device
...
def separable_fourier_filter(N, sigma):
    ...
    k = (torch.arange(-N, N, device=device) ** 2) / (2 * sigma**2)
...
class GeneralGaussianBlur3D_notperiodic(torch.nn.Module):
    ...
    def forward(self, u):
        ...
        u_pad = torch.nn.functional.pad(
            u[None, None],
            (self.pad_y, self.pad_y, self.pad_x, self.pad_x, self.pad_z, self.pad_z),
        ...
        u = u_pad[
            self.pad_z : -self.pad_z, self.pad_x : -self.pad_x, self.pad_y : -self.pad_y
        ]
...
def my_custom_GradHess(mode):
    ...
    class GradHessianFunc(torch.autograd.Function):
        ...
        def backward(ctx, d_grad, d_H_diag, d_H_off):
            ...
            padding(
                d_pad, d_H_diag, GradHessianFunc.first_backward
            )  # d_pad <- d_H_diag
    ...
    class GradHessian(torch.nn.Module):
        def __init__(self, Z, X, Y):
            ...
            self.register_buffer(
                "u_pad", torch.zeros(Z + 2, X + 2, Y + 2, device=device)
            )
            self.register_buffer("grad", torch.zeros(Z, X, Y, 3, device=device))
            self.register_buffer("H_diag", torch.zeros(Z, X, Y, 3, device=device))
            self.register_buffer("H_off", torch.zeros(Z, X, Y, 3, device=device))

            self.register_buffer(
                "d_pad", torch.zeros(Z + 2, X + 2, Y + 2, 3, device=device)
            )
            self.register_buffer("d_u", torch.zeros(Z, X, Y, device=device))

        def forward(self, u):

            return GradHessianFunc.apply(
                u, self.u_pad, self.grad, self.H_diag, self.H_off, self.d_pad, self.d_u
            )
