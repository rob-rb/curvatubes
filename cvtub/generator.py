...
def _generate_shape(
    ...
    for (
        x
    ) in params:  # if one of the parameters is space-dependent then spatialized = True
        ...
        if flow_type == "L2":
            ...
            if True or n_evals > 0:
                ...
                if border_distance_weight > 0:
                    ...
                    exp = 1
                    E += (
                        torch.linalg.vector_norm(diff[border, :, :]) ** exp
                        * border_distance_weight
                    )
                    E += (
                        torch.linalg.vector_norm(diff[:, border, :]) ** exp
                        * border_distance_weight
    ...
    def take_snapshot():
        ...
        if cond_take_snapshot(iteration) and first_snapshot == True:
            ...
            save_nii(
                u.detach().cpu().numpy(), snapshot_folder + title_end, compressed=True
            )
    ...
    if nan_OK:
        ...
        ax[1, 0].plot(
            fill_curve_nb * np.arange(len(grad_L1mean_curve)), grad_L1mean_curve
        )
        ...
        if False:
            ...
            ax[1].plot(
                fill_curve_nb * np.arange(len(E_curve)), E_curve + 1e-6 - E_curve.min()
            )
            ax[1].set_yscale("log")
            ax[1].set_title("E curve + 1e-6 - minimal value")

            fig.savefig(
                snapshot_folder + "Curves/" + exp_title + title + " E_curves.png"
            )
            plt.show()
