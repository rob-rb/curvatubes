...
def slices(u, figsize=(12, 4), rescale=True, cmap="gray", save=False, title=""):
    ...
    ax[1].imshow(
        np.asarray(u[Z // 2], dtype=np.float64), vmin=vmin, vmax=vmax, cmap=cmap
    )
    ax[2].imshow(
        np.asarray(u[Z - 1], dtype=np.float64), vmin=vmin, vmax=vmax, cmap=cmap
    )
    fig.tight_layout()
    if title != "":
        fig.suptitle(title)
...
def random_init(size, prop=0.2):  # prop between 0 and 1
    ...
    if isinstance(size, int):
        N = size
        v0 = torch.rand((N, N, N), device=device)  # .type(dtype)
    else:
        v0 = torch.rand(size, device=device)  # .type(dtype)
    v0 = 1.0 * (v0 <= prop)
    return 2 * v0 - 1
...
def init_holed_cubes(
    size, N_cubes=1, option="", mode="replicate", R=20, r=10, med=False, med_rad=0
):
    ...
    if N_cubes == 1:
        cubes[
            Z // 2 - R : Z // 2 + R, X // 2 - R : X // 2 + R, Y // 2 - R : Y // 2 + R
        ] = 1
    ...
    cubes = 2.0 * cubes - 1
    v0 = torch.Tensor(cubes, device=device)
    return v0
...
def place_balls(Z, X, Y, ZZ, XX, YY, coords, r_pix, mode, dist, return_tensor=True):
    ...
    balls = 2.0 * (balls != 0) - 1
    if return_tensor:
        balls = torch.Tensor(balls, device=device)
    return balls
...
def coords_n_balls(size, n, option=""):
    ...
    if n == 2:
        if option == "apart":
            return np.array([[Z // 2, Z // 2], [X // 2, X // 2], [Y // 4, 3 * Y // 4]])
        if option == "close":
            return np.array([[Z // 2, Z // 2], [X // 2, X // 2], [Y // 3, 2 * Y // 3]])
    if n == 3:
        if option == "right":
            return np.array(
                [
...
def manual_softplus(u, xi_bis=1e-6):
    ...
    return (
        xi_bis + u * (u > 0) + xi_bis * torch.log(1 + torch.exp(-torch.abs(u) / xi_bis))
    )
